...
// ---------------------------------------------------------------------------
// Types (inline to avoid ~/types alias in tsx)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Mock data (copied from app/utils/mock-data.ts)
// ---------------------------------------------------------------------------

const categories: SeedCategory[] = [
  { $id: 'cat-utilities', name: 'Utilities & Scripts', slug: 'utilities-scripts', icon: 'i-lucide-code', sortOrder: 1 },
  { $id: 'cat-buttons', name: 'Buttons', slug: 'buttons', icon: 'i-lucide-mouse-pointer-click', sortOrder: 2 },
  { $id: 'cat-scroll', name: 'Scroll Animations', slug: 'scroll-animations', icon: 'i-lucide-scroll', sortOrder: 3 },
  { $id: 'cat-sliders', name: 'Sliders & Marquees', slug: 'sliders-marquees', icon: 'i-lucide-gallery-horizontal', sortOrder: 4 },
  { $id: 'cat-video', name: 'Video & Audio', slug: 'video-audio', icon: 'i-lucide-play-circle', sortOrder: 5 },
  { $id: 'cat-cursor', name: 'Cursor Animations', slug: 'cursor-animations', icon: 'i-lucide-pointer', sortOrder: 6 },
  { $id: 'cat-text', name: 'Text Animations', slug: 'text-animations', icon: 'i-lucide-type', sortOrder: 7 },
  { $id: 'cat-gimmicks', name: 'Gimmicks', slug: 'gimmicks', icon: 'i-lucide-sparkles', sortOrder: 8 },
  { $id: 'cat-navigation', name: 'Navigation', slug: 'navigation', icon: 'i-lucide-menu', sortOrder: 9 },
  { $id: 'cat-dropdowns', name: 'Dropdowns & Information', slug: 'dropdowns-information', icon: 'i-lucide-chevron-down', sortOrder: 10 }
]
...
const resourceCodes: SeedResourceCode[] = [
  {
    $id: 'code-1',
    resourceId: 'res-1',
...
async function documentExists(
  ...
  try {
    await db.getDocument(DATABASE_ID, collectionId, documentId)
    return true
  } catch {
    return false
  }
...
async function main() {
  ...
  for (const code of resourceCodes) {
    ...
    if (!REPLACE_MODE && await documentExists(db, COLLECTIONS.RESOURCE_CODE, $id)) {
      ...
      await db.createDocument(
        DATABASE_ID,
        COLLECTIONS.RESOURCE_CODE,
        $id,
        data,
        [Permission.read(Role.any())]
      )
      console.log(`  \u2713 Created resource-code: ${code.$id}`)
    }
  }
